...
func (s *MigrationService) recoverFromDirtyState(ctx context.Context, dirtyVersion int) error {
	s.logger.Printf("Dirty migration state detected at version %d; refusing automatic force recovery", dirtyVersion)
	return fmt.Errorf("database migration is dirty at version %d; manual repair is required", dirtyVersion)
}
